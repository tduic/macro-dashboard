...
1. **Markets dashboard** — the key numbers you watch (equities, rates, FX, energy,
   ...
   change and **click-through historical charts**.
2. **News + economic calendar** — an aggregated RSS news rail and an upcoming
...
| GET | `/api/news` | deduped, reverse-chron news `[{title, source, url, publishedAt, category}]` |
| GET | `/api/calendar` | next ~14 days of FRED releases (empty/disabled without a key) |
...
|----------|--------|-------------|
| **Equities** | yfinance | SPY, QQQ, IWM, DIA, ^VIX |
